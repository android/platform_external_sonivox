...
class SonivoxTest : public ::testing::TestWithParam<tuple</*fileName*/ string,
  ...
  public:
    ...
    virtual void SetUp() override {
        ...
        mAudioBuffer = new EAS_PCM[mEASConfig->mixBufferSize * mEASConfig->numChannels];
...
bool SonivoxTest::renderAudio() {
    ...
    if (count < 0 || count != mEASConfig->mixBufferSize) {
...
TEST_P(SonivoxTest, DecodePauseResumeTest) {
    ...
    ASSERT_EQ(currentPosMs, mAudioplayTimeMs / 2) << "Invalid position reached after pause";
    ...
    ASSERT_GT(currentPosMs, mAudioplayTimeMs / 2) << "Invalid position after resuming";
