...
class SonivoxTest : public ::testing::TestWithParam<tuple</*fileName*/ string,
  ...
  public:
    ...
    virtual void SetUp() override {
        ...
        mPCMBuffer = new EAS_PCM[mPCMBufferSize];
...
bool SonivoxTest::renderAudio() {
    ...
    EAS_RESULT result = EAS_Render(mEASDataHandle, pcm, mPCMBufferSize, &count);
    if (result != EAS_SUCCESS) {
        ALOGE("Failed to render audio");
        return false;
    }
    if (count < 0 || count != mPCMBufferSize) {
        ALOGE("Failed to write %ld bytes of data to buffer", mPCMBufferSize);
        return false;
    }
...
TEST_P(SonivoxTest, SeekTest) {
    ...
    status = seekToLocation(mAudioplayTimeMs + kRandomSeekOffsetMs);
    ASSERT_FALSE(status) << "Invalid seek position: " << mAudioplayTimeMs + kRandomSeekOffsetMs;
}

TEST_P(SonivoxTest, DecodePauseResumeTest) {
    // go to middle of the audio
    EAS_RESULT result = EAS_Locate(mEASDataHandle, mEASStreamHandle, mAudioplayTimeMs / 2, false);
    ASSERT_EQ(result, EAS_SUCCESS) << "Failed to locate to location(ms): " << mAudioplayTimeMs / 2;
    ...
    ASSERT_LE(currentPosMs, mAudioplayTimeMs) << "No data to render";

    ASSERT_EQ(currentPosMs, mAudioplayTimeMs / 2) << "Invalid position after resuming";
