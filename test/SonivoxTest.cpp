...
bool SonivoxTest::renderAudio() {
    EAS_I32 count = -1;
    EAS_PCM *pcm = new EAS_PCM[mEASConfig->mixBufferSize];
    if (!pcm) {
        ALOGE("Failed to allocate memory of size: %ld", mEASConfig->mixBufferSize);
        return false;
    }
...
TEST_P(SonivoxTest, DecodePauseResumeTest) {
    ...
    ASSERT_GE(currentPosMs, mAudioplayTimeMs / 2) << "Invalid position after resuming";
