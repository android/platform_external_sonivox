...
bool SonivoxTest::renderAudio(EAS_I32 bufferSize) {
    ...
    if (mPCMBuffer == nullptr) {
        ALOGE("Failed to allocate a memory of size %ld: ", bufferSize);
        return false;
    }
...
TEST_P(SonivoxTest, DecodeTest) {
    ...
    while (1) {
        ...
        ASSERT_NE(pcmBuffer, nullptr) << "Failed to allocate a memory of size: " << bufferSize;
        ...
        int32_t numBytes = fwrite(pcmBuffer, 1, numBytesOutput, filePtr);
